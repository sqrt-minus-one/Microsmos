...
W32_InitOpenGL(HINSTANCE h_instance)
{
    ...
    int dummy_pixel_format = 0;
    {
        ...
        gl_dummy_game_state = wglCreateContext(dummy_hdc);
        wglMakeCurrent(dummy_hdc, gl_dummy_game_state);
        W32_LoadWGLFunctions();
        wglMakeCurrent(0, 0);
        wglDeleteContext(gl_dummy_game_state);
    }
    ...
    //- NOTE(fakhri): initialize real context
    {
        ...
        if(w32_gl_ctx)
        {
            wglMakeCurrent(w32_device_context, w32_gl_ctx);
